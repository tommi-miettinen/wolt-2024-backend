...
class DeliveryFeeConfig(BaseModel):
    ...
    CART_VALUE_THRESHOLD_FOR_SURCHARGE: Annotated[int, Field(gt=0, validate_default=True)] = 1000
    FREE_CART_VALUE_THRESHOLD: Annotated[int, Field(gt=0, validate_default=True)] = 20000
    ...
    def validate_rush_hours(cls, value: int, values: Dict[str, Any]):
        if "RUSH_HOUR_START_HOUR" in values and value <= values["RUSH_HOUR_START_HOUR"]:
            raise ValueError("RUSH_HOUR_END_HOUR must be greater than RUSH_HOUR_START_HOUR")
        return value
...
@validate_call
def is_rush_hour(time: datetime):
    return (
...
@validate_call
def get_small_order_surcharge(cart_value: Annotated[int, Field(gt=0)]):
    return max(config.CART_VALUE_THRESHOLD_FOR_SURCHARGE - cart_value, 0)


@validate_call
def get_distance_fee(delivery_distance: Annotated[int, Field(gt=0)]):
    if delivery_distance <= config.BASE_DISTANCE:
        return config.BASE_FEE

    intervals_over_base = math.ceil(
        (delivery_distance - config.BASE_DISTANCE) / config.ADDITIONAL_DISTANCE
    )
    return int(config.BASE_FEE + (intervals_over_base * config.ADDITIONAL_FEE))


@validate_call
def get_bulk_fee(number_of_items: Annotated[int, Field(gt=0)]):
    fee = 0
...
def get_total_delivery_fee(request: DeliveryFeeRequest):
    if request.cart_value >= config.FREE_CART_VALUE_THRESHOLD:
        return 0
    ...
    if is_rush_hour(request.time):
        fee = int(fee * config.RUSH_HOUR_FEE_MULTIPLIER)
